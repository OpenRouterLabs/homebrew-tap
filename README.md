...
This internal repository contains Homebrew formulae for OpenRouter tools.

Its Git remote is `git@github.com:OpenRouterTeam/homebrew-tap.git`. Homebrew exposes it as the `OpenRouterTeam/tap` tap.
...
Authenticate to GitHub over SSH, then add the internal tap and install a formula:

```sh
brew tap OpenRouterTeam/tap git@github.com:OpenRouterTeam/homebrew-tap.git
brew install <name>
```

GitHub organization access protects the internal tap repository. Release artifacts need their own authentication if they are private.
...
A Homebrew formula runs code on employee computers. Treat each formula change as a privileged software-distribution change.
